...
test('ID chip copies NPC ID to the clipboard', async (t) => {
  ...
  const idChip = await screen.findByRole('button', { name: /ID: npc_ember_fl4m3/i });
  fireEvent.click(idChip);

  await waitFor(() => {
    assert.deepEqual(clipboardWrites, ['npc_ember_fl4m3']);
  });
});
