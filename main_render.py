...
from core.utils import render_hash
...
def _write_wav(path: Path, audio: np.ndarray, sr: int, comment: str | None = None) -> None:
    ...
    The implementation writes a minimal RIFF/WAV file and, if ``comment`` is
    provided, stores it in an ``INFO``/``ICMT`` chunk so that the render hash is
    ...
    fmt_chunk = struct.pack("<HHIIHH", 1, n_channels, sr, byte_rate, block_align, 16)

    info_chunk = b""
    if comment:
        c_bytes = comment.encode("utf-8")
        pad = b"\x00" if len(c_bytes) % 2 else b""
        icmt = b"ICMT" + struct.pack("<I", len(c_bytes)) + c_bytes + pad
        list_data = b"INFO" + icmt
        info_chunk = b"LIST" + struct.pack("<I", len(list_data)) + list_data

    data_chunk = b"data" + struct.pack("<I", len(byte_data)) + byte_data
    riff_size = 4 + (8 + len(fmt_chunk)) + len(info_chunk) + len(data_chunk)
    with path.open("wb") as fh:
        fh.write(b"RIFF" + struct.pack("<I", riff_size) + b"WAVE")
        fh.write(b"fmt " + struct.pack("<I", len(fmt_chunk)) + fmt_chunk)
        if info_chunk:
            fh.write(info_chunk)
        fh.write(data_chunk)
...
if __name__ == "__main__":
    ...
    render_id = render_hash(spec.to_dict(), args.seed, style, cfg, git_commit)
    ...
    if args.bundle:
        ...
        _write_wav(mix_path, mix_audio, 44100, comment=render_id)
        ...
        if args.bundle_stems:
            ...
            for name, audio in rendered.items():
                ...
                _write_wav(stem_path, audio, 44100, comment=render_id)
    ...
    print(json.dumps({"event": "render_complete", "render_hash": render_id}))
